...
# import Network.sd_scripts.library.huggingface_util as huggingface_util
import Network.sd_scripts.library.model_util as model_util
...
def load_target_model(name_or_path, weight_dtype, device="cpu"):
    ...
    if save_stable_diffusion_format:
        ...
        def save_sd():
            ...
            model_util.save_stable_diffusion_checkpoint(
                v2, ckpt_file, text_encoder, unet, src_path, epoch_no, global_step, save_dtype, vae
            )
        ...
        def remove_du(old_epoch_no):
            out_dir_old = os.path.join(output_dir, EPOCH_DIFFUSERS_DIR_NAME.format(model_name, old_epoch_no))
...
def save_sd_model_on_train_end(
    ...
    if save_stable_diffusion_format:
        ...
        model_util.save_stable_diffusion_checkpoint(
            False, ckpt_file, text_encoder, unet, src_path, epoch, global_step, save_dtype, vae
        )
        # if args.huggingface_repo_id is not None:
        ...
        model_util.save_diffusers_checkpoint(
            False, out_dir, text_encoder, unet, src_path, vae=vae, use_safetensors=use_safetensors
        )
        # if args.huggingface_repo_id is not None:
