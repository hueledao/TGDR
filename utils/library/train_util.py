...
def add_sd_models_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--v2", action="store_true", default=False, help="load Stable Diffusion v2.0 model / Stable Diffusion 2.0のモデルを読み込む")
    parser.add_argument(
    ...
    #     type=str,
    #     default="/media/EXT0/daole/VCM_Proposed/pretrained/sd-v1-5-inpainting.ckpt",
...
def add_optimizer_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--learning_rate", type=float, default=4.8e-5, help="learning rate / 学習率")
    parser.add_argument(
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    # parser.add_argument("--output_dir", type=str, default="Weights/Weights_UNet_IP_MaskedIMG/", help="directory to output trained model / 学習後のモデル出力先ディレクトリ")
    ...
    parser.add_argument(
        "--save_every_n_epochs", type=int, default=1, help="save checkpoint every N epochs / 学習中のモデルを指定エポックごとに保存する"
    )
    parser.add_argument(
        "--save_n_epoch_ratio",
        type=int,
    ...
    parser.add_argument("--max_train_steps", type=int, default=1000, help="training steps / 学習ステップ数")
    parser.add_argument(
        "--max_train_epochs",
        type=int,
        default=100,
...
def add_dataset_arguments(
    ...
    # dataset common
    parser.add_argument("--train_data_dir", type=str, default="../VCM_Proposed/data/New/synthesis/", help="directory for train images / 学習画像データのディレクトリ")
    parser.add_argument(
...
def load_target_model_testing(name_or_path, weight_dtype, device="cpu"):
    # name_or_path = args.pretrained_model_name_or_path
    name_or_path = os.readlink(name_or_path) if os.path.islink(name_or_path) else name_or_path
    load_stable_diffusion_format = os.path.isfile(name_or_path)  # determine SD or Diffusers
    if load_stable_diffusion_format:
        print("load StableDiffusion checkpoint")
        text_encoder, vae, unet = model_util.load_models_from_stable_diffusion_checkpoint(False, name_or_path, device)
    else:
...
def load_target_model(args: argparse.Namespace, weight_dtype, device="cpu"):
    ...
    if load_stable_diffusion_format:
        ...
        try:
            pipe = StableDiffusionPipeline.from_pretrained(name_or_path, tokenizer=None, safety_checker=None)
            # pipe = StableDiffusionInpaintPipeline.from_pretrained(name_or_path, tokenizer=None, safety_checker=None)
        except EnvironmentError as ex:
            print(
                f"model is not found as a file or in Hugging Face, perhaps file name is wrong? / 指定したモデル名のファイル、またはHugging Faceのモデルが見つかりません。ファイル名が誤っているかもしれません: {name_or_path}"
            )
