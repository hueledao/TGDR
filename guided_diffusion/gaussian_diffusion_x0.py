...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, IC = None, is_latentspace = None, text_embedding = None,
    ):
        """
        ...
        # model_output = model(x, self._scale_timesteps(t), **model_kwargs) #org GDP
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, pred_xstart, t, is_latentspace = False, model_kwargs=None):
        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, x.shape)
        scale = (1 - alpha_bar).sqrt()
        """
    ...
    def p_sample(
        ...
        if approach2 & t[0]!= 0:
            ...
            x_prev = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise #x_{t-1} 
            ...
            noise = th.randn_like(x)
            nonzero_mask = (
                (t[0] != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
            )  # no noise when t == 0
            
            if cond_fn is not None:
                out["mean"] = self.condition_mean(
                    cond_fn, out, x, out["pred_xstart"], t, is_latentspace = is_latentspace, model_kwargs=model_kwargs
                )
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
        # if is_latentspace:
        #     with th.no_grad():
        #         sample = vae.decode(1 / 0.18215 * sample).sample 
        # print(t)
        # if (writer is not None) & (t.item()==0):
        #     # print(t.item()==0)
        # # if writer is not None:
        #     # print(writer)
        #     # print("%.4f"%out["mean"].mean(dim=(1,2,3)))
        #     # writer.writerow("Now the file has more content!!!!!!")
        #     mean_val = float(out["mean"].mean(dim=(1,2,3)).item())
        #     mean_val =  format(mean_val, ".4f") 
        #     # print('writing.................................')
        #     writer.writerow([mean_val])
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                yield out
                img = out["sample"]
                # os.makedirs(os.path.join('/media/EXT0/daole/sd_scripts/GenerativeDiffusionPrior/scripts/Imgs/VVC_xt/', '000000_bw'), exist_ok=True)
                # utils.save_image(img, 'Imgs/VVC_xt/000000_bw/step_%d.png' % (t*20), nrow=1, normalize=True)
                # utils.save_image(img, 'Imgs/JPEG_xt/000011/step_%d.png' % (t*20), nrow=1, normalize=True)
