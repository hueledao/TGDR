...
from .gaussian_diffusion_x0 import GaussianDiffusion
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def condition_mean(self, cond_fn, *args, **kwargs):
        # return super().condition_mean(self._wrap_model_s(cond_fn), *args, **kwargs)
    ...
    def _wrap_model_s(self, model):
        if isinstance(model, _WrappedModel_s):
            return model
        return _WrappedModel_s(
            model, self.timestep_map, self.rescale_timesteps, self.original_num_steps
        )
...
class _WrappedModel:
    ...
    def __call__(self, x, ts, text_embedding = None):
        ...
        if text_embedding is not None:
            return self.model(x, new_ts, text_embedding).sample
        else:
            return self.model(x, new_ts)
class _WrappedModel_s:
    def __init__(self, model, timestep_map, rescale_timesteps, original_num_steps):
        self.model = model
        self.timestep_map = timestep_map
        self.rescale_timesteps = rescale_timesteps
        self.original_num_steps = original_num_steps        
    def __call__(self, x, ts, scale):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype) # self.timestep_map = [0...1000]
        new_ts = map_tensor[ts] # new_ts = ts = tensor [999], 
        if self.rescale_timesteps:
            new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
        return self.model(x, new_ts, scale)
